...
class _ExplodeState extends State<Explode> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    Future.delayed(Duration(seconds: 2)).then((_) {
      if (widget.instantExplode && widget.shakeBeforeExploding)
        _controller.forward();
    });
    super.initState();
  }
  ...
  createParticles() {
    for (int i = 0; i < particleCount; i++) {
      particles.add(DropPath(
          initialPosition: Offset(random.nextInt(size.width.toInt()).toDouble(),
              random.nextInt(size.height.toInt()).toDouble()),
          initialRadius: random.nextDouble() * 2,
  ...
  Vector3 shake() {
    ...
    double x = widget.shakeDistance *
        random.nextDouble() *
        (random.nextBool() ? -1 : 1);
    double y = widget.shakeDistance *
        random.nextDouble() *
        (random.nextBool() ? -1 : 1);
    print(Size(x, y));
    ...
    return Container(
      ...
      child: CustomPaint(
          ...
          painter: ParticlesPainter(
              dropPaths: particles, t: _explodeAnimation.value),
